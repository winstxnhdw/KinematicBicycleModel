# cython: language_level=3, cdivision=True, auto_pickle=False, binding=False

from libc.math cimport atan2, cos, fmax, fmin, sin, tan, remainder, M_PI
...
cdef inline double clip(const double value, const double min_value, const double max_value) noexcept nogil:
    return fmin(max_value, fmax(min_value, value))


cdef inline double normalise_angle(const double angle) noexcept nogil:
    return remainder(angle, 2 * M_PI)


cdef inline double compute_angular_velocity(
    const double velocity,
    const double steer,
    const double wheelbase
) noexcept nogil:
    return velocity * tan(steer) / wheelbase
